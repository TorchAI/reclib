...
# This base import is so we can refer to reclib.data.Token in `sanitize()` without creating
# circular dependencies.
from reclib.common.checks import log_pytorch_version_info
from reclib.common.params import Params
from reclib.common.tqdm import Tqdm
from reclib.common.tee_logger import TeeLogger
...
def sanitize(x: Any) -> Any:
    ...
    from reclib.data.tokenizers.token import Token
...
def prepare_global_logging(
    ...
    Note that this function does not set the logging level, which is set in ``reclib/run.py``.
...
def import_submodules(package_name: str) -> None:
    ...
    Primarily useful so that people using reclib as a library
